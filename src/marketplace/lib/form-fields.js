...
export const isMultiValue = (type) => type === 'multiselect';
 ...
 */
export function validateAnswer(field, raw) {
  const multi = isMultiValue(field.type);
  const empty = multi ? !raw?.length : raw === '' || raw === null || raw === undefined;
 ...
 */
export function readAnswers(answers = {}, fields = [], locale = 'ar') {
  const byKey = new Map(fields.map((f) => [f.field_key, f]));

  return Object.entries(answers ?? {})
    .map(([key, entry]) => {
      const field = byKey.get(key);
      const stored = entry && typeof entry === 'object' && !Array.isArray(entry) ? entry : null;
