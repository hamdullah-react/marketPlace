 ...
 */
/** An offer row as the database stores it (listing_offers). */
...
/** The same offer after shapeOffer(), which is what reaches the browser. */
export type ShapedOffer = {
  discountType?: string | null;
  discountValue?: number | null;
  [key: string]: unknown;
};

/** Accepted everywhere a clock is passed — a Date, or epoch millis. */
export type NowInput = Date | number;
...
export function findLiveOffer(
  offers: RawOffer[] | null | undefined,
  now: NowInput = Date.now(),
): RawOffer | null {
...
export function priceWithOffer(
  ...
  shaped: ShapedOffer | null | undefined,
): number | null {
 ...
 */
export function offerStatus(offer: RawOffer | null | undefined, now: NowInput = Date.now()) {
