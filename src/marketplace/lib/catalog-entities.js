...
export const nameFieldOf = (key) => ENTITIES[key]?.nameField ?? 'name';
export const iconFieldOf = (key) => ENTITIES[key]?.iconField ?? 'icon_url';
export const sequenceFieldOf = (key) => ENTITIES[key]?.sequenceField ?? 'sequence';

/** Columns to select — only what the table actually renders. */
export function selectFor(key) {
 ...
 */
export function lacksColumn(key, column) {
  return (ENTITIES[key]?.lacksColumns ?? []).includes(column);
}
