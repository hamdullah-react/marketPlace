...
const BY_KEY = Object.fromEntries(FORM_STYLES.map((s) => [s.key, s]));

/** The theme a vendor who has changed nothing gets, for one preset. */
export function defaultTheme(styleKey = 'classic') {
  const preset = BY_KEY[styleKey] ?? FORM_STYLES[0];

  const numbers = Object.fromEntries(
    NUMERIC_TOKENS.map((n) => [n.key, preset.numbers[n.key] ?? n.def])
  );
 ...
 */
export function normaliseTheme(theme, styleKey = 'classic') {
  const t = theme && typeof theme === 'object' ? theme : {};
  const base = defaultTheme(styleKey);

  const out = {
    accent: HEX_COLOR.test(t.accent ?? '') ? t.accent.toLowerCase() : base.accent,
  };
  ...
  for (const c of COLOR_FIELDS) {
    out[c.key] = HEX_COLOR.test(t[c.key] ?? '') ? t[c.key].toLowerCase() : null;
  }
 ...
 */
export function themeVars(theme, styleKey = 'classic') {
  ...
  for (const c of COLOR_FIELDS) if (t[c.key]) vars[c.varName] = t[c.key];

  return vars;
}

/** For the callers that only care about the accent. */
export const accentVars = (hex) => themeVars({ accent: hex });
 ...
 */
export function formStyle(key, theme) {
...
const WIDTH_BY_KEY = Object.fromEntries(WIDTHS.map((w) => [w.key, w]));

export function widthSpan(key) {
