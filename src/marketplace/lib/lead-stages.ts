...
export function sellerStage(key: string | null | undefined) {
  return (key ? SELLER_BY_KEY[key] : undefined) ?? SELLER_BY_KEY.new;
}

export function buyerStage(key: string | null | undefined) {
  const table = BUYER_STAGES as Record<string, (typeof BUYER_STAGES)['new'] | undefined>;
  return (key ? table[key] : undefined) ?? BUYER_STAGES.new;
}
...
export function canBuyerCancel(stage: string): boolean {
 ...
 */
export function reachedStep(stage: string): number {
