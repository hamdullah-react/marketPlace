...
/**
 * A row as it comes back from one of this file's selects.
 *
 * Deliberately loose, and the three reasons are all real: `listings.attributes`
 * is jsonb, so `attributes.year` is unknowable to the compiler; each query in
 * here embeds a different set of joins; and attachOffers() bolts
 * `listing_offers` on after the read. Naming one Row type would describe none
 * of them accurately while making the other two unassignable.
 *
 * The queries themselves are still fully checked — that happens at
 * `.from(...).select(...)`, against db/types.ts. This is only the shape the
 * facet and card shaping below passes around afterwards.
 */
type CarRow = Record<string, any>;

const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

async function slugSiblings(table: string, values?: (string | number)[] | null) {
  ...
  const all = ((family ?? []) as CarRow[]).map((r) => String(r.id));

  return all.length ? all : wanted;
}

/**
 * Everything /marketplace/cars can be narrowed by.
 *
 * Every field is optional — this comes straight off a query string, where the
 * absence of a filter is the normal case. The list-valued ones accept a slug or
 * a uuid interchangeably (see slugSiblings), which is why they are not typed as
 * uuid strings.
 */
export type CarFilters = {
  ...
  specRanges?: Record<string, { min?: number; max?: number }> | null;
  locale?: string;
  sort?: string;
  limit?: number;
  offset?: number;
...
export async function getCars(opts: CarFilters = {}) {
  const {
  ...
  const [column, ascending] = SORTS[sort as keyof typeof SORTS] ?? SORTS.newest;
  query = query.order(column as string, { ascending: ascending as boolean, nullsFirst: false });
  ...
  if (hasOffer || offer?.length) {
    ...
    const keep = filtered.filter((r: CarRow) => {
      ...
      return list.some((o: any) => wanted.has(o.label?.en) || wanted.has(o.label?.ar));
    });
 ...
 */
/**
 * The brand/condition lock a category or brand page applies to its facets.
 * Values may be ids OR slugs — see the filter below for why both are matched.
 */
export type FacetScope = { brand?: string[]; condition?: string[] } | null;

export async function getCarFacets(locale = 'ar', scope: FacetScope = null) {
  /**
  ...
  const all = (data ?? []) as CarRow[];
  const rows: CarRow[] = scope
    ? all.filter((r: CarRow) => {
  ...
  const tally = (pick: (row: CarRow) => unknown) => {
  ...
  const [offerRows, specFacets, kinds] = await Promise.all([
    ...
    getSpecFacets(ids as string[], locale).catch(() => []),

    kindsPromise,
  ]);

  const byLabel = new Map<string, CarRow>();
  const covered = new Set<string>();
  for (const o of offerRows as CarRow[]) {
  ...
  const kindFacets = kinds
    .filter((k: CarRow) => !DEDICATED.has(k.kind))
    .map((k: CarRow) => {
      ...
      return {
        ...
        values: (k.options ?? [])
          .filter((o: any) => counts.has(o.slug))
          .map((o: any) => ({ value: o.slug, name: o.name, count: counts.get(o.slug) })),
      };
    })
    .filter((k: CarRow) => k.values.length > 0);
  ...
  return {
    ...
    years: tally((r: CarRow) => r.attributes?.year).sort((a, b) => Number(b.value) - Number(a.value)),
    maxMileage: mileages.length ? Math.max(...mileages) : 0,
    cities: tally((r: CarRow) => r.city),
    conditions: tally((r: CarRow) => r.attributes?.condition),
    transmissions: tally((r: CarRow) => r.attributes?.transmission),
    fuels: tally((r: CarRow) => r.attributes?.fuel),
    ...
    seats: tally((r: CarRow) => r.attributes?.seats).sort((a, b) => Number(a.value) - Number(b.value)),
  };
}
...
export async function getNavData() {
  ...
  return { offerCount: new Set(offerRows.map((o: any) => o.listing_id)).size };
}
...
export async function getBrandsIndex(locale = 'ar') {
  ...
  for (const row of rowsRes.data ?? []) {
    const slug = row.brand_id ? brandSlug.get(row.brand_id) : null;
    if (!slug) continue;
    ...
    const attrs = (row.attributes ?? {}) as CarRow;
    if (attrs.year) s.years.add(String(attrs.year));
    stats.set(slug, s);
  }
 ...
 */
export async function getBrandBySlug(slug: string, locale = 'ar') {
  if (!slug) return null;
