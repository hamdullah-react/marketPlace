...
import {
  ENTITIES, selectFor, nameFieldOf, sequenceFieldOf,
 ...
 */
async function vendorScope(vendorId, table) {
 ...
 * round trip.
 */
export async function listEntity(key, {
  parentId, kind, category, q, page = 1, pageSize = DEFAULT_PAGE_SIZE, vendorId = null,
} = {}) {
 ...
 */
export async function listParentOptions(key, vendorId = null) {
  const entity = ENTITIES[key];
  if (!entity?.parent) return [];

  const parentKey = entity.parent.entity;
  const parent = ENTITIES[parentKey];
  ...
  return (data ?? []).map((r) => ({ id: r.id, slug: r.slug, name: r[field] }));
}
...
export async function listSpecCategories(vendorId = null) {
  ...
  const seen = new Map();
  for (const row of data ?? []) {
    const key = row.category_name?.en || row.category_name?.ar;
...
export async function getEntity(key, id) {
  const entity = ENTITIES[key];
  if (!entity) throw new Error(`Unknown catalog entity "${key}"`);
 ...
 */
export async function countDependents(key, id) {
  const entity = ENTITIES[key];
...
export async function countListingRefs(key, id) {
