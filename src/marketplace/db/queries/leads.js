...
export async function getVendorLeads(
  vendorId,
  { stage, unread = false, q = '', limit = 20, offset = 0 } = {}
) {
  ...
  if (stage === 'deleted') {
    ...
    if (stage === 'open') query = query.in('stage', OPEN_STAGE_KEYS);
    else if (stage && LEAD_STAGE_KEYS.includes(stage)) query = query.eq('stage', stage);
  }
...
export async function getLeadCounts(vendorId) {
  const empty = {
 ...
 */
export async function countUnreadLeads(vendorId) {
...
export async function getLead(vendorId, leadId) {
 ...
 */
export async function getOpenLead(vendorId, listingId, userId) {
 ...
 */
export async function getBuyerLeads(userId, { limit = 50 } = {}) {
  if (!userId) return [];
