 ...
 */
export async function getVendorFormFields(vendorId, { activeOnly = true } = {}) {
  if (!vendorId) return [];
  ...
  if (error) {
    ...
    return (retry.data ?? []).map((f) => ({ ...f, width: 'full', tab_id: null }));
  }
 ...
 */
export async function getVendorFormStyle(vendorId) {
 ...
 */
export async function getVendorFormTabs(vendorId) {
  if (!vendorId) return [];
