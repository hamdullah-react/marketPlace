import { getMarketplaceDb, type MarketplaceDb } from '@/marketplace/db/client';
 ...
 */
async function installedTemplates(db: MarketplaceDb, vendorId: string) {
  const { data } = await db
    .from('catalog_template_installs')
    .select('template')
    .eq('vendor_id', vendorId);

  return [...new Set((data ?? []).map((r: LooseRow) => r.template))];
}
 ...
 */
export async function getVendorMedia(vendorId: string, opts = {}) {
  ...
  try {
    ...
    if (String(err instanceof Error ? err.message : err).includes('42703')) {
...
async function readMedia(
  vendorId: string,
  { kind, limit = 100, offset = 0 }: { kind?: string; limit?: number; offset?: number } = {},
  select: string,
) {
  ...
  const [mine, theirs] = await Promise.all([
    kind ? own.eq('kind', kind as Enum<'media_kind'>) : own,
    shared
      ? kind
        ? shared.eq('kind', kind as Enum<'media_kind'>)
        : shared
      : Promise.resolve({ data: [] as LooseRow[], count: 0, error: null }),
  ]);
  ...
  const items = ([...(mine.data ?? []), ...(theirs.data ?? [])] as LooseRow[]).filter((row) => {
 ...
 */
export async function getMediaFolders(vendorId: string) {
  if (!vendorId) return [];
...
export async function getMediaByIds(ids = []) {
  ...
  const { data, error } = await getMarketplaceDb()
    .from('media_assets').select(BASE_SELECT).in('id', ids);
...
export async function getMediaStats(vendorId: string) {
...
export async function getListingVariants(listingId: string) {
  if (!listingId) return [];
 ...
 */
export function normalizeVariant(row: LooseRow, locale = 'ar') {
  if (!row) return null;
