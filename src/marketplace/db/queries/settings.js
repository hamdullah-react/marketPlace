...
export async function updateVendor(vendorId, patch) {
  ...
  const { data, error } = await getMarketplaceDb()
    .from('vendors').update(patch).eq('id', vendorId).select(SELECT).single();
...
export async function getBackups(vendorId, { limit = 20 } = {}) {
  if (!vendorId) return [];
 ...
 * export.
 */
export async function collectVendorData(vendorId) {
...
export async function getVendorDataCounts(vendorId) {
  const db = getMarketplaceDb();
  const count = async (table, column = 'vendor_id') => {
    try {
