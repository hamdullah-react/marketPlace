...
import type { LocalizedValue } from '@/marketplace/lib/listing';
...
export async function getAttributes(kind?: string | null) {
...
export async function getKindsWithOptions() {
  ...
  const [groups, metaRows] = await Promise.all([
    ...
    getMarketplaceDb()
      ...
      .eq('active', true)
      // The rejection handler is the SECOND argument rather than a chained
      // .catch(): a PostgrestBuilder is a thenable, not a Promise, so it has
      // no .catch to chain.
      .then(({ data, error }) => (error ? [] : data ?? []), () => []),
  ]);

  const meta = new Map(metaRows.map((row) => [row.slug, row]));
...
export async function getCardKinds(locale = 'ar') {
  ...
  return defs.map((d) => ({
    ...
    values: Object.fromEntries(
      Object.entries(d.values).map(([slug, name]) => [
        slug,
        localized(name as LocalizedValue, locale) || slug,
      ])
    ),
  }));
}
