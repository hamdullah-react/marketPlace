...
export async function getVendorListings(
  vendorId: string,
  {
    ...
    limit = 50,
    offset = 0,
  }: { state?: string; q?: string; limit?: number; offset?: number } = {},
) {
  ...
  if (state) query = query.eq('state', state as Enum<'listing_state'>);
...
export async function getSellerStats(vendorId: string) {
  ...
  const countBy = (state: Enum<'listing_state'>) =>
  ...
  const [live, draft, pending, rejected, sold, leads, views] = await Promise.all([
    ...
    db.from('leads').select('id', { count: 'exact', head: true })
      .eq('vendor_id', vendorId)
      .in('stage', OPEN_STAGES as Enum<'lead_stage'>[])
      .is('deleted_at', null),
 ...
 */
export async function getVendorPayouts(vendorId: string, { limit = 24, offset = 0 } = {}) {
  if (!vendorId) return { items: [], total: 0 };
 ...
 */
export async function getPayoutTotals(vendorId: string) {
 ...
 */
export async function getVendorReviews(vendorId: string, { limit = 24, offset = 0 } = {}) {
  if (!vendorId) return { items: [], total: 0 };
 ...
 */
export async function getOfferableListings(vendorId: string) {
  if (!vendorId) return [];
