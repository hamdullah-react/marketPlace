 ...
 */
async function loadListings(vendorId: string) {
 ...
 */
async function withBrandNames(rows: LooseRow[]) {
  if (!rows.length) return rows;

  const { data, error } = await getMarketplaceDb()
    .from('car_brands')
    .select('id, name')
    .in('id', rows.map((r: LooseRow) => r.name));

  if (error) return rows;

  const names = new Map((data ?? []).map((b) => [b.id, b.name]));
  return rows.map((r: LooseRow) => ({ ...r, id: r.name, name: names.get(r.name) ?? r.name }));
}
 ...
 */
export async function getSellerAnalytics(vendorId: string, { days = 30 } = {}) {
  ...
  // ── by state ─────────────────────────────────────────────────────────────
  const byState = listings.reduce<Record<string, number>>((map, l) => {
  ...
  const series = [];
  const counts = listings.reduce<Record<string, number>>((map, l) => {
  ...
  const leadCounts = leads.reduce<Record<string, number>>((map, l) => {
    if (!l.created_at) return map;
    const key = dayKey(new Date(l.created_at));
    map[key] = (map[key] ?? 0) + 1;
    return map;
  ...
  const tally = (rows: LooseRow[], key: string) => {
    const map = rows.reduce((acc, r: LooseRow) => {
      const k = r[key];
      if (!k) return acc;
      acc[k] = (acc[k] ?? 0) + 1;
      return acc;
